# Ectocontrol Modbus Adapter v2

Home Assistant custom integration to talk to the Ectocontrol Modbus Adapter v2 (RS-485 Modbus RTU).

Features
- Polls a block of boiler registers (0x0010..0x0026) and exposes sensors, binary sensors, numbers and switches.
- Uses `modbus-tk` for RTU/serial communication.
- Configurable via the Integrations UI (Config Flow). Supports multiple adapters/boilers.
...
3. Open Settings → Devices & Services → Add Integration → search for "Ectocontrol Modbus Adapter v2".
