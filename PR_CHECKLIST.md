...
- [ ] Implement `Climate` entity (primary thermostat)
  - Files to add/modify:
    - [custom_components/ectocontrol_modbus/entities/climate.py](custom_components/ectocontrol_modbus/entities/climate.py#L1)
    - Wire into [custom_components/ectocontrol_modbus/__init__.py](custom_components/ectocontrol_modbus/__init__.py#L1) forward entry setups
  - Tests:
...
- [ ] Add `button` entities for adapter commands (reboot, reset errors)
  - Files to add/modify:
    - [custom_components/ectocontrol_modbus/entities/button.py](custom_components/ectocontrol_modbus/entities/button.py#L1)
...
- [ ] Implement full set of switch/number entities per plan
  - Files to add/modify:
    - `custom_components/ectocontrol_modbus/entities/switch.py` (expand for DHW enable, Reboot button fallback if needed)
    - `custom_components/ectocontrol_modbus/entities/number.py` (add CH min/max, DHW setpoint, max modulation)
  - Tests:
...
- [ ] Add adapter-type and adapter-reboot-code sensors
  - Files to modify:
    - `custom_components/ectocontrol_modbus/boiler_gateway.py` — add `get_adapter_type()` and `get_adapter_reboot_code()` helpers reading MSB/LSB of `REGISTER_STATUS`.
    - `custom_components/ectocontrol_modbus/entities/sensor.py` — add sensor descriptions and exposure.
  - Tests:
...
- [ ] Add centralized `RetryPolicy` and integrate with coordinator/protocol
  - Files to add/modify:
    - `custom_components/ectocontrol_modbus/retry.py` — implement `execute_with_retry()` (exponential backoff) per plan.
...
- [ ] Narrow `config_flow` slave ID range to plan (1–32) or document reason to allow up to 247
  - Files to modify:
    - `custom_components/ectocontrol_modbus/config_flow.py`
  - Tests:
...
- [ ] Replace standalone service handlers with HA `button` platform (optional, see previous task)
  - Files to modify:
    - `custom_components/ectocontrol_modbus/__init__.py` — remove or keep service registration as compatibility shim.
  - Tests:
